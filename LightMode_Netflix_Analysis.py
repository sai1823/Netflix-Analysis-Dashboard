...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            dbc.Card([
                html.H1("Netflix Analysis ",style={"color": "#1C1C1C","font-weight": "bold",'backgroundColor': 'F0F0F0','text-align': 'center',"font-family": "Bebas Neue"} )
            ],style={"backgroundColor": 'F0F0F0'},className='mb-2'),
        ], width=12),
    ],style={"backgroundColor": '#F0F0F0'},className='mb-2 mt-2'),
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardImg(src="https://media.giphy.com/media/JnvHE3lTHPr3WrSsrl/giphy.gif",style={"width":"100%", "height":"110px"})
            ],style={"backgroundColor": '#F0F0F0'},className='mb-2'),
        ],style={"backgroundColor": '#F0F0F0'}, width=2),
        dbc.Col([
            dbc.Card([
    dbc.CardBody([
        dbc.Row([ 
            ...
            dbc.Col([
                html.P("Type",style={"color": "#1C1C1C",}),
                dcc.Dropdown(id="type", clearable=False,
                             value="All",
                             options=[{'label':'All', 'value':'All'}] + [{'label':x, 'value':x} for x in df['type'].unique()],style={"color": "#1C1C1C",'backgroundColor':'#F0F0F0'})
            ], width=2),

            dbc.Col([
                html.P("Number of Countries",style={"color": "#1C1C1C",}),
                dcc.Dropdown(id="num_countries", clearable=False,
                             
                             options=[{'label':i, 'value':i} for i in range(1, 21)],style={"color": "#1C1C1C",'backgroundColor':'#F0F0F0'},
                             value=20)
            ], width=2),
                            
            dbc.Col([
                html.P("Released Year",style={"color": "#1C1C1C",}),
                dcc.Dropdown(id="Year", clearable=False,
                             value=max(df['release_year']),
                             options=year_options,style={"color": "#1C1C1C",'backgroundColor':'#F0F0F0'})
                             
            ], width=2),

            dbc.Col([
                html.P("Country",style={"color": "#1C1C1C",}),
                dcc.Dropdown(id="country", clearable=False,
                             ...
                             value=['All'],
                             style={"color": "#1C1C1C",'backgroundColor':'#F0F0F0'})
            ], width=3),

            dbc.Col([
                html.P("Genre",style={"color": "#1C1C1C",}),
                dcc.Dropdown(id="Genre", clearable=False,
                             value=['All'],
                             multi=True,
                             options=[{'label':'All', 'value':'All'}] +[{'label': i, 'value': i} for i in df['genre'].sort_values().unique()],style={"color": "#1C1C1C",'backgroundColor':'#F0F0F0'}
)
            ], width=3),
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H6("Total Content available",style={"color": "#1C1C1C",}),
                    html.H5(id="content",children="000",style={"color": "#1C1C1C","font-weight": "bold"}),
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H6("Total released in summer(May)",style={"color": "#1C1C1C",}),
                    html.H5(id="summer_content",children="000",style={"color": "#1C1C1C","font-weight": "bold"}),
                ], style={'textAlign':'center'})
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H6("Avg Duration",style={"color": "#1C1C1C"}),
                    html.H5(id="duration",children="000",style={"color": "#1C1C1C","font-weight": "bold"}),
                    ], style={'textAlign':'center'})
    ...
    dbc.Row([
        dbc.Col([
            ...
            ],style={'height': '100%',"backgroundColor": '#F0F0F0','border': '1px solid #F0F0F0'}),
        ], width=4),
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dcc.Graph(id='tree-map', figure={}, config={'displayModeBar': False}),
                ])
            ],style={"backgroundColor": '#F0F0F0','border': '1px solid #F0F0F0'}),
        ], width=5),
...
], style={"backgroundColor": '#F0F0F0'},fluid=True)
...
def update_histogram(type_val, num_countries_val, year_val, country_val, genre_val):
    ...
    fig.update_layout(
        ...
        yaxis=dict(gridcolor='#E4E4E4'),title_x=0.5       
    )
    return fig
...
def update_tree_map(type_val, num_countries_val, year_val, country_val, genre_val):
    filtered_df = df.copy()
    if type_val != "All":
        filtered_df = filtered_df[filtered_df["type"] == type_val]
    if year_val is not None:
...
def update_word_cloud(type_val, num_countries_val, year_val, country_val, genre_val):
    ...
    wordcloud = WordCloud(background_color='#F9F9F9', collocations=False, color_func=lambda *args, **kwargs: "#E74646").generate(text)
    ...
    fig_wordcloud.update_layout(
        ...
        title_font=dict(size=20, color="#1C1C1C"),title_x=0.5,      
    )
